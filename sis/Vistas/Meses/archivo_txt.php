...
foreach($arrays1 as $fila)
{
    ...
    $rep="SELECT CODIGOSIAF, CODIGO, DETALLE
    FROM renumeraciones
    ORDER BY CODIGO;";
...
function getColumnRange($start, $end)
{
    ...
    for ($i = $startIndex; $i <= $endIndex; $i++) {
        $range[] = Coordinate::stringFromColumnIndex($i);
    }
...
$filas = range(5, 200);
$columnas = getColumnRange('A', 'CF');

$contenidoTxt = '';
$columnWidths = array_fill_keys($columnas, 0);

foreach ($filas as $fila) {
    foreach ($columnas as $columna) {
        $celdaValor = $hoja->getCell($columna . $fila)->getValue();
        $columnWidths[$columna] = max($columnWidths[$columna], strlen($celdaValor));
    }
}
...
foreach ($filas as $fila) {
    ...
    foreach ($columnas as $columna) {
        $celdaValor = $hoja->getCell($columna . $fila)->getValue();
        if (isNullOrWhitespace($celdaValor)) {
            $celdaValor = '';
        } else {
            $celdaValor = removeWhitespace($celdaValor);
        }
        $celdaValor = str_pad($celdaValor, $columnWidths[$columna] + 1, ' ', STR_PAD_RIGHT);
        $linea .= $celdaValor;
    }
    $contenidoTxt .= rtrim($linea) . "\n";
}
$nombreArchivo = 'mes_de_' . $monthName . '_' . $mesac['anio'] . '.txt';
file_put_contents($nombreArchivo, $contenidoTxt);
header('Content-Description: File Transfer');
header('Content-Type: application/octet-stream');
header('Content-Disposition: attachment; filename="' . $nombreArchivo . '"');
readfile($nombreArchivo);
// Eliminar el archivo de texto después de descargarlo
