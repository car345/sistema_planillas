...
<div id="page_pdf">
	...
	<tr><th style="justify-content:center; font-size:13px;"   colspan="23" >Descuentos</th> </tr>
...
?></th>
		...
		<th style="font-size:11px;" colspan="11	"  ><span style='border-bottom: 0.2px solid black;'>Descuentos</span></th>	
...
<td style="font-size:11px; "  >T. descuento</td>
	...
	<tr ><td style="font-size:11px; " colspan=""  >CATEGORÍA</td>
		...
		echo '<td style="font-size:9px;  colspan="1"  > Otros Desc.</td>'; 
		?> 
...
<td style="font-size:11px; " colspan=""  ></td>
	...
	<tr><td style="font-size:11px; " colspan=""  >CTA-CTE</td>
		...
		<td style="font-size:11px;  " colspan="5"  > CARGO</td>
			...
			<?php
				...
				$query="SELECT f.id_datperso,f.NUMERO_DOC,f.NOMBRE,f.APELLIDOS,f.CARGO,f.id_categori,f.CTA_CTE,f.id_afps,
				p.REGISTRO,af.DESC as afps_desc,categ.DESC as categori_desc
				FROM persxmes p inner join datperso f on p.REGISTRO=f.id_datperso inner join afps af on f.id_afps=af.id_afps
				inner join categori categ on f.id_categori=categ.id_categori
				where p.REGMES='$meses' AND f.id_modali='$modal' ";
	...
	$elemento=0;
			foreach($tabla2Datos[$cliente['id_datperso']] as $importe)
			{
				...
				if($g==7)
				{
					echo ' <tr> <td style="font-size:11px; " colspan="1"  > '.$cliente['categori_desc'].' </td>'; 	
