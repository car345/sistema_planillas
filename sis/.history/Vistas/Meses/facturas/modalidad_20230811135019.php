...
<div id="page_pdf">
	...
	foreach($arraydesc as $desc){
		$codigo =$desc['DESCT'];
					...
					$queryx1 = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots3 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigo' AND m.id_modali='1'");
					$queryy1 = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots4 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigo' AND m.id_modali='3'");
					$queryz1 = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots5 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigo' AND m.id_modali='21'");
					$resultx1=$queryx->fetch_assoc();
					$resulty1=$queryy->fetch_assoc();
					$resultz1=$queryz->fetch_assoc();
