...
<div id="page_pdf">
	...
	foreach($arraydesc as $desc){
		$codigo =$desc['DESCT'];
					...
					$queryx = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots3 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigo' AND m.id_modali='1'");
