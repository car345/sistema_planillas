	...
	<tr><td style="font-size:11px; " colspan=""  >CTA-CTE</td>
		...
		<td style="font-size:11px;  " colspan=""  > CARGO</td>
			...
			<?php
				...
				$query="SELECT f.id_datperso,f.NUMERO_DOC,f.NOMBRE,f.APELLIDOS,f.CARGO,f.id_categori,f.CTA_CTE,f.id_afps,p.REGISTRO,af.id_afps as afps_desc
FROM persxmes p inner join datperso f on p.REGISTRO=f.id_datperso inner join afps af on f.id_afps=af.id_afps where p.REGMES='$meses' AND f.id_modali='$modal' ";
	...
	$elemento=0;
			...
			echo '<tr><td style="font-size:11px;  " colspan="1"  > '.$cliente['CTA_CTE'].' </td>
					<td style="font-size:11px; font-weight: normal;" colspan="1"  > '.$cliente['afps_desc'].' </td>
