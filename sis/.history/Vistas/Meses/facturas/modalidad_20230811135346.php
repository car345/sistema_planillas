...
<div id="page_pdf">
	...
	foreach($arraydesc as $desc){
		$codigos =$desc['DESCT'];
					...
					$queryy2 = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots4 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigos' AND m.id_modali='3'");
					$queryz3 = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots5 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigos' AND m.id_modali='21'");
					
					$resultx1=$queryx1->fetch_assoc();
					$resulty1=$queryy2->fetch_assoc();
