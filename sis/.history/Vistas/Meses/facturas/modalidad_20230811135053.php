...
<div id="page_pdf">
	...
	foreach($arraydesc as $desc){
		$codigos =$desc['DESCT'];
					...
					$queryx1 = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots3 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigos' AND m.id_modali='1'");
					$queryy1 = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots4 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigos' AND m.id_modali='3'");
					$queryz1 = mysqli_query($conn,"SELECT sum(p.IMPORTE)as tots5 FROM reportxplanilla p inner join datperso d on p.REGPERSO=d.id_datperso inner join modali m on d.id_modali=m.id_modali WHERE p.REGMES='$mes' AND p.DESCT='$codigos' AND m.id_modali='21'");
					$resultx1=$queryx1->fetch_assoc();
					$resulty1=$queryy1->fetch_assoc();
					$resultz1=$queryz1->fetch_assoc();
	...
	?>
		...
		<tr>
					<td  style="font-size:12px;" colspan="2"><?php echo $row['CODIGO']; ?></td>
					<td style="font-size:12px" colspan="10"><?php echo $row['DESCT']; ?></td>
					<td style="font-size:12px" colspan="3"><?php echo number_format($resultx1['tots'],2); ?></td>
					<td colspan="5"> </td>
					<td style="font-size:12px" colspan="15"><?php echo number_format($resultz1['tots2'],2); ?></td>
					<td style="font-size:12px" colspan="15"><?php echo number_format($resulty1['tots1'],2); ?></td>
