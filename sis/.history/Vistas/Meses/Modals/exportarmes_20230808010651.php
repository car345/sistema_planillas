...
const idMesPorBoton = new Map(); // Mapa para almacenar id_mes por botón

function obtenerMesDeBoton(boton) {
  const fila = boton.closest("tr");
  // Asumiendo que el mes está en algún lugar de la fila
  const mes = fila.querySelector(".mes-element").textContent;
  return mes;
}

document.querySelectorAll(".boton-planilla").forEach(boton => {
  boton.addEventListener("click", function() {
    const mes = obtenerMesDeBoton(boton);
    console.log("Mes:", mes);

    idMesPorBoton.set(boton, mes); // Almacena el id_mes en el mapa

    const modal = new bootstrap.Modal(document.getElementById("miModal"));
    modal._element.setAttribute("data-mes", mes);
    modal.show();
  });
});
...
document.querySelectorAll(".modal").forEach(modal => {
  modal.addEventListener("show.bs.modal", function(event) {
    ...
    const mesIdEnFila = fila.getAttribute("data-mes-id");
    
    modal.setAttribute("data-mes-id", mesIdEnFila);
  });

  modal.addEventListener("shown.bs.modal", function(event) {
    const mesIdEnModal = modal.getAttribute("data-mes-id");
