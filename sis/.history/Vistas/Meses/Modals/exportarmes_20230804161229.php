...
<span>
  <span class="fw-bold">TIPO DE PLANILLA</span>
    <select  data-id="<?php echo  $data["id_meses"];?>" name="tipp<?php echo  $data["id_meses"];?>" id="tipp<?php echo  $data["id_meses"];?>" class="form-select">
    <option value="">Seleccione una opcion</option>
  ...
  <span class="fw-bold">FILTRAR</span>
    <select name="" id="filtrar<?php echo  $data["id_meses"];?>" onchange="habilitarInput()" class="form-select">
    <option value="">Seleccione una opcion</option>
  ...
  <div class="tab-content" id="descuentoley<?php echo  $data["id_meses"];?>" style="display: none;">
    ...
    <p>Descripción de la opción Planilla Mensual.</p>
     ...
     function habilitarInput(event) {
      var modalidadInput = event.target;
      var modalidadId = modalidadInput.getAttribute('data-id');
      var inputHabilitado = document.querySelector('[data-id="' + modalidadId + '_habilitado"]');

      if (modalidadInput.value === 'opcion1') {
